...
static ERL_NIF_TERM load_dict(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    enif_get_string(env, argv[0], model_dict, len, ERL_NIF_LATIN1);

    segment.init(model_dict);
...
{
    {"cut", 1, cut},
    {"load_dict", 1, load_dict}
};
}
ERL_NIF_INIT(Elixir.ExJieba.HMMSegment, nif_funcs, NULL, NULL, NULL, NULL)
